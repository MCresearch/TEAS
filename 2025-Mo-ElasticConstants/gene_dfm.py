...
for ii in range(n_dfm):
    ...
    if run_type == "abacus":
        ...
        stru = read("POSCAR",format='vasp')
        n_atoms = len(set(stru.get_chemical_symbols()))
        atom_mass = []
        pseudo_dict = {}
        orb_dict = {}
        ...
        for idx, line in enumerate(lines):
            if "ATOMIC_SPECIES" in line:
                for i in range(n_atoms):
                    element = lines[idx+i+1].split()[0]
                    atom_mass.append(float(lines[idx+i+1].split()[1]))
                    pseudo_dict[element] = lines[idx+i+1].split()[2]
            if "NUMERICAL_ORBITAL" in line:
                for i in range(n_atoms):
                    element = lines[idx+i+1].split('_')[0]
                    orb_dict[element] = lines[idx+i+1].split()[0]
        if not orb_dict:
            #stru.to("stru", "STRU", mass=atom_mass, pp_file=pseudo)
            write("STRU",stru,format='abacus',pp=pseudo_dict)
        else:
            #stru.to("stru", "STRU", mass=atom_mass, pp_file=pseudo, numerical_orbital=orb)
            write("STRU",stru,format='abacus',pp=pseudo_dict, basis=orb_dict)
        os.remove("POSCAR")
