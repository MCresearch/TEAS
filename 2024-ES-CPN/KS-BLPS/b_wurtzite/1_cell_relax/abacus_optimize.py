...
class ABACUS:
    def __init__(self, minNatom, maxNatom, ecut, stru_dir, calculation, kinetic, use_kernel, kernel_dir, kernel_prefix, xc):
        ...
        if self.xc == 0:
            ...
            self.pseudo = {"Al":'al.lda.lps', "Mg":'mg.lda.lps', "Li":'li.lda.lps.1', "Si":'si.lda.lps', "Ga":'ga.lda.lps', "In":'in.lda.lps', "P":'p.lda.lps', "As":'as.lda.lps', "Sb":'sb.lda.lps'}
        elif self.xc == 1: 
            self.pseudo = {"Al":'al.gga.psp', "Mg":'mg.gga.psp', "Li":'li.gga.psp.1', "Si":'si.gga.psp', "Ga":'ga.gga.psp', "In":'in.gga.psp', "P":'p.gga.psp', "As":'as.gga.psp', "Sb":'sb.gga.psp'}
...
mixing_type		pulay\n\
mixing_beta		0.7\n".format(
                self.calculation, self.symmetry_prec, self.pseudo_dir, nbands, self.ecut, self.kspacing))
...
of_tole         2e-6\n\
of_read_kernel  {2}\n\
of_kernel_file  {3}\n".format(ntype, self.kinetic, self.use_kernel, self.kernel_dir + self.kernelfile))
    ...
    def create_files(self):
        ...
        with open('jobs', 'w') as job:
            ...
            for i in range(self.Nstru):
                ...
                job.write("cd $temp_path\{0} && {1} > ../log/{2} && echo \"{3} done\"\n".format(path, self.abacus, path, path))
...
if __name__ == '__main__':
    ...
    # for sub_dir in ["0_LiMg", "1_MgAl", "2_LiAl", "3_LiMgAl"]:
    #     total_dir = stru_dir + sub_dir
    #     if not os.path.exists(sub_dir): os.mkdir(sub_dir)
    #     os.chdir(sub_dir)
    abacus = ABACUS(minNatom, maxNatom, ecut, stru_dir, calculation, kinetic, use_kernel, kernel_dir, kernel_prefix, xc)
    os.chdir(work_dir)
