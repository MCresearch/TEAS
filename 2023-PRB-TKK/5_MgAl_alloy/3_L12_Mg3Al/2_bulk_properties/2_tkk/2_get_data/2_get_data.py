...
class Profess:
    def __init__(self):
        ...
        self.structures = ['fcc']
        self.constands = [[4.354913142046369],
                          [4.369345084886834],
                          [4.322874541623090],
                          [4.318784238633173],
                          [4.319014382374561],
        ...
        self.pseudo = ['al.lda.recpot', 'mg.lda.recpot']
        ...
        self.work_dir = "/home/dell/1_work/TKK_abacus/5_MgAl_alloy/3_L12_Mg3Al/2_bulk_properties/2_tkk/2_get_data"
        self.id = ['Al', 'Mg', 'Mg', 'Mg']
        self.N = 11
        ...
        for i in range(len(self.kernelfiles)):
            ...
            for each in self.pseudo:
                os.system('cp {0} .'.format(self.pseudo_dir+each))
            self.create_files()
            self.cal_e_v()
        os.chdir(self.work_dir)
        ...
        with open('jobs', 'w') as job:
            for i in range(len(self.structures)):
                for j in range(self.N):
                    ...
                    os.mkdir(path)
                    self.create_inpt(path+'/'+self.id[0]+'.inpt')
                    self.create_ion(path+'/'+self.id[0]+'.ion', cell, self.position[i])
                    job.write(self.pPROFESS + ' ' + path + '/' + self.id[0] + '\n')
    ...
    def cal_e_v(self):
        ...
        for i in range(len(self.structures)):
            for j in range(self.N):
                outfile = './{0}{1}/{2}.out'.format(self.structures[i], j, self.id[0])
                try:
