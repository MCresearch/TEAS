...
class PROFESS:
    def __init__(self):
        ...
        # self.structures = ['111', '211', '221', '222', '333', '444']
        self.structures = ['555', '666', '777']
        self.unit_cell = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        self.unit_position = np.array([[0, 0, 0], [0.5, 0.5 ,0], [0.5, 0, 0.5], [0, 0.5, 0.5]])
        self.cell = []
        ...
        self.work_dir = "/home/dell/1_work/TKK_abacus/1_EFTKK_Al/6_analyze/4_vacancy_energy"
        self.id = 'Al'
        ...
        os.chdir(self.work_dir)
        self.workbook.save('vacancy_energy_fcc_new.xls')
    ...
    def get_cells(self):
        for each in self.structures:
            ...
            self.fcc_position.append(position[:])
    ...
    def create_files(self):
        with open('jobs', 'w') as job:
            for i in range(len(self.structures)):
                ...
                self.create_inpt(path+'/'+self.id+'_fcc.inpt')
                self.create_ion(path+'/'+self.id+'_fcc.ion', cell, self.fcc_position[i])
                job.write(self.pPROFESS + ' ' + path + '/' + self.id + '_fcc\n')

    def calculate(self):
        # perform structure optimization
        # e_list = np.zeros((2,self.N * len(self.structures)))
        etot_list = np.zeros(len(self.structures))
        efcc_list = np.zeros(len(self.structures))
        ...
        for i in range(len(self.structures)):
            ...
            fccfile = './{0}/{1}_fcc.out'.format(self.structures[i], self.id)
            try:
                ...
                get_fcc_e = subprocess.Popen(args="grep 'TOTAL ENERGY' %s|tr -s ' '|cut -d ' ' -f5" % fccfile,
                                            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
                fcc_e = float(get_fcc_e.stdout.read())  # maybe wrong
            except:
                total_e = 1e5
                fcc_e = 1e5
            etot_list[i] = total_e
            efcc_list[i] = fcc_e
        evacancy_list = etot_list - self.natom/(self.natom+1) * efcc_list
