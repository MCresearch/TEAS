...
double BESSJY(double x, double xnu, double *rj, double *ry, double *rjp, double *ryp)
{
	const int XMIN = 2;
    ...
    if (x <= 0.0 || xnu < 0.0)
	{
//		WARNING_QUIT("BESSJY","bad arguments");
		exit(0);
	}
    ...
    *ryp = xnu * xi * rymu - ry1;
	...
	// mohan update 2013-09-12
	cout << " please check, something wrong " << endl;
	exit(0);
	return 0;
}

double Spherical_Bessel_7(const int n, const double &x)
{
	if(x==0)
	{
		if(n!=0) return 0;
	...
	{
		exit(0);
//		WARNING_QUIT("Spherical_Bessel_7","bad arguments in sphbes");
	}
	...
	for(int i=0; i<msh && n<num; i++)
	{
		if(jl[i]*jl[i+1] < 0.0)
		{
			...
			while(acc > epsilon)
			{
				...
				Spherical_Bessel(100,rad,1,l,jl_new);
...
void allocate_Bessel(double DR, double RCUT, int N_BESSEL, int L_USED, double jle[][1610])
{
	assert(DR>0.0 && DR<1.0);
	assert(RCUT>0.0 && RCUT<1000);
	assert(N_BESSEL>0 && N_BESSEL < 1000);

    int nr; // number of real space mesh points.
	double* eigenvalue; // eigenvalues of Spherical Bessel functions.
	double* r; // real space meth
	double* rab; // real space dr
	const double tolerence = 1.0e-12;

	nr = static_cast<int>(RCUT/DR);
	if(nr%2==0) nr++;
	assert(nr<=1000);

	// this eigenvalue will be used in many functions.
	eigenvalue = new double[N_BESSEL];
	r = new double[nr];
	rab = new double[nr];
	...
	// find the eigenvalues of Jlq.
	Spherical_Bessel_Roots(N_BESSEL, L_USED, tolerence, eigenvalue, RCUT);

	for(int ie=0; ie<N_BESSEL; ie++)
	{
		Spherical_Bessel( nr, r, eigenvalue[ie], L_USED, jle[ie]);
	}
    delete[] r;
    delete[] rab;
    delete[] eigenvalue;
