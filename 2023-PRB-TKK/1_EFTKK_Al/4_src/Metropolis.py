...
class Metropolis:
    ...
    def get_target(self, settings):
        targetfile = np.loadtxt(settings.targetfile, skiprows=3).T
        # [total energy, kinetic energy, hartree energy, natoms]
        self.natoms = targetfile[3]
        target = targetfile[:3]
        # V0 = settings.v_list[5]
        # V0, target_B = profess.fit(
        #     settings.v_list, target[0,:11], settings.v_list[0], settings.v_list[-1], V0)[1:]
        # target.append(np.array([target_B]))
        # return target, V0
        return target
    ...
    def get_residual(self, coef, settings, create_kernel, profess):
        ...
        temp = profess.cal_e_v(settings.name, settings.structures)
        # B = profess.fit(
        #     settings.v_list, temp[0], settings.v_list[0], settings.v_list[-1], self.V0)[-1]
        # temp.append(np.array([B]))
        # diff = np.array([sum(abs(temp[i] - self.target[i]))
        #                  for i in range(len(self.target))]) / self.natoms
    ...
    def metropolis(self, settings, create_kernel, profess):
        ...
        for i in range(settings.nT):
            ...
            for j in range(settings.nsteps):                #increase steps
